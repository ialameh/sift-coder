import { createMCPToolMetadata, } from "../../model/ToolDefinition.js";
/**
 ...
 */
export async function buildBuildPrompt(details) {
    const { projectType, features, techStack, requirements, } = details;
 ...
 */
export async function buildFixPrompt(details) {
    const { issue, expectedBehavior, actualBehavior, errorMessages, context, } = details;
 ...
 */
export async function buildTestPrompt(details) {
    const { target, testType, scenarios, edgeCases, } = details;
 ...
 */
export async function buildDocumentPrompt(details) {
    const { targetType, audience, includeSections, } = details;
 ...
 */
export async function buildImprovePrompt(details) {
    const { target, improvementGoals, constraints, } = details;
 ...
 */
export async function buildRefactorPrompt(details) {
    const { target, goals, patterns, } = details;
 ...
 */
export async function buildDebugPrompt(details) {
    const { error, reproductionSteps, context, recentChanges, } = details;
 ...
 */
export async function buildInvestigatePrompt(details) {
    const { query, focusAreas, } = details;
 ...
 */
export function getQuestionsForCommand(command) {
    const questions = {
        build: [
 ...
 */
export function explainPrompt(command, details) {
 ...
 */
export async function improvePrompt(existingPrompt, additionalContext) {
 ...
 */
function restructureFixPrompt(prompt) {
 ...
 */
function restructureBuildPrompt(prompt) {
    const lines = prompt.split('\n').filter(l => l.trim());
    // Check if already has sections
