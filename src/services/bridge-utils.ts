...
export class BridgeUtilsService {
   ...
   */
  async detectProjectType(dir?: string): Promise<ProjectType> {
   ...
   */
  async extractTechStack(dir?: string): Promise<TechStack> {
    const targetDir = dir || this.projectRoot;
    ...
    if (await FileUtils.exists(packageJsonPath)) {
      ...
      const frameworks = Object.keys(deps).filter(d =>
        frameworkPatterns.some(f => d.includes(f))
      );
   ...
   */
  async extractApiEndpoints(_dir?: string): Promise<ApiEndpoint[]> {
   ...
   */
  async findEntryPoints(dir?: string): Promise<string[]> {
    const targetDir = dir || this.projectRoot;
   ...
   */
  async getProjectStructure(dir?: string): Promise<Record<string, number>> {
