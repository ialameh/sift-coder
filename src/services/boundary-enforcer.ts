...
import { StateManager, Boundaries } from './state-manager.js';
...
export class BoundaryEnforcer {
   ...
   */
  async checkFile(filePath: string): Promise<BoundaryCheckResult> {
    ...
    // Check if protected
    const isProtected = boundaries.protected.some(pattern =>
      FileUtils.match(normalizedPath, pattern)
    );
    ...
    // Check if modifiable
    const isModifiable = boundaries.modifiable.some(pattern =>
      FileUtils.match(normalizedPath, pattern)
    );
   ...
   */
  async checkFiles(filePaths: string[]): Promise<Map<string, BoundaryCheckResult>> {
    const results = new Map<string, BoundaryCheckResult>();
   ...
   * Set boundaries
   */
  async setBoundaries(boundaries: Omit<Boundaries, 'last_check'>): Promise<void> {
    const fullBoundaries: Boundaries = {
      ...boundaries,
   ...
   */
  async addModifiable(pattern: string): Promise<void> {
    const boundaries = await this.stateManager.loadBoundaries();
   ...
   */
  async addProtected(pattern: string): Promise<void> {
    const boundaries = await this.stateManager.loadBoundaries();
   ...
   */
  async getBoundaries(): Promise<Boundaries | null> {
   ...
   * Verify blast radius
   */
  async verifyBlastRadius(files: string[]): Promise<boolean> {
