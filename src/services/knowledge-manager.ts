...
export class KnowledgeManagerService {
   ...
   * Add a pattern
   */
  async addPattern(pattern: Omit<Pattern, 'id' | 'addedAt'>): Promise<string> {
    await this.init();

    const patternsFile = PathUtils.join(this.knowledgeDir, 'patterns.json');
    const patterns = await FileUtils.readJSON<Pattern[]>(patternsFile);
   ...
   * Add a gotcha
   */
  async addGotcha(gotcha: Omit<Gotcha, 'id' | 'addedAt'>): Promise<string> {
    await this.init();

    const gotchasFile = PathUtils.join(this.knowledgeDir, 'gotchas.json');
    const gotchas = await FileUtils.readJSON<Gotcha[]>(gotchasFile);
   ...
   * Add a decision
   */
  async addDecision(decision: Omit<Decision, 'id' | 'addedAt'>): Promise<string> {
    await this.init();

    const decisionsFile = PathUtils.join(this.knowledgeDir, 'decisions.json');
    const decisions = await FileUtils.readJSON<Decision[]>(decisionsFile);
  ...
  async getPatterns(): Promise<Pattern[]> {
    ...
    return FileUtils.readJSON<Pattern[]>(patternsFile);
  }

  /**
   * Get all gotchas
   */
  async getGotchas(): Promise<Gotcha[]> {
    const gotchasFile = PathUtils.join(this.knowledgeDir, 'gotchas.json');
    if (!await FileUtils.exists(gotchasFile)) {
      return [];
    }
    return FileUtils.readJSON<Gotcha[]>(gotchasFile);
  }
  ...
  async getDecisions(): Promise<Decision[]> {
    ...
    return FileUtils.readJSON<Decision[]>(decisionsFile);
  }
   ...
   */
  async searchGotchas(query: string): Promise<Gotcha[]> {
    const gotchas = await this.getGotchas();
    const lowerQuery = query.toLowerCase();
    return gotchas.filter(g =>
      g.issue.toLowerCase().includes(lowerQuery) ||
