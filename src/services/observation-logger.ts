...
let currentPhase: 'PLANNING' | 'CODING' | 'QA' | 'DONE' = 'PLANNING';
let currentAgent: string = 'siftcoder-orchestrator';
let currentFeature: string = '';
let currentSubtask: string = '';
...
export function updateContext(updates: {
  ...
  if (updates.agent) currentAgent = updates.agent;
  if (updates.feature) currentFeature = updates.feature;
  if (updates.subtask) currentSubtask = updates.subtask;
  if (updates.iteration) currentIteration = updates.iteration;
 ...
 */
function validateObservation(entry: ObservationEntry): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
function extractLearning(observation: ObservationEntry): ObservationEntry['learning'] {
  const learning = {
    concepts: [] as string[],
    facts: [] as string[],
    gotchas: [] as string[],
 ...
 */
export async function logObservation(observation: ObservationEntry): Promise<void> {
  try {
...
export async function queryObservations(filters?: {
  ...
  try {
    ...
    let observations: ObservationEntry[] = lines.map(line => JSON.parse(line));
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  switch (command) {
    ...
    case 'query': {
      const filters: any = {};
      if (process.argv[3] === '--agent') filters.agent = process.argv[4];
      if (process.argv[3] === '--phase') filters.phase = process.argv[4];
      if (process.argv[3] === '--concept') filters.concept = process.argv[4];
