...
export class IndexCodebaseService {
   ...
   * Find all code files
   */
  async findCodeFiles(searchPath: string = '.'): Promise<CodeFile[]> {
   ...
   * Index codebase
   */
  async index(searchPath: string = '.'): Promise<IndexResult> {
    await this.init();
   ...
   * Get stored index
   */
  async getIndex(): Promise<{ files: CodeFile[]; indexedAt: string } | null> {
    const indexFile = PathUtils.join(this.indexDir, 'file-index.json');
    ...
    return FileUtils.readJSON<{ files: CodeFile[]; indexedAt: string }>(indexFile);
  }

  /**
   * Search files by pattern
   */
  async searchFiles(query: string): Promise<CodeFile[]> {
