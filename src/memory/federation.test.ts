...
describe('federatedSearch', () => {
  it('searches across all consented workspaces and merges results', async () => {
    ...
    const hits = await federatedSearch('summary', null, { home, decayTauMs: 1e15, k: 5 });
  ...
  it('omits non-consented workspaces from results', async () => {
    ...
    const hits = await federatedSearch('summary', null, { home, decayTauMs: 1e15 });
  ...
  it('respects workspacePrefix filtering', async () => {
    await mkWs('aaa1', true);
    await mkWs('bbb2', true);
    const hits = await federatedSearch('summary', null, { home, workspacePrefix: 'aaa', decayTauMs: 1e15 });
  ...
  it('respects maxWorkspaces', async () => {
    ...
    await mkWs('w2', true);
    await mkWs('w3', true);
    const hits = await federatedSearch('summary', null, { home, maxWorkspaces: 1, decayTauMs: 1e15 });
  ...
  it('honors the global k limit after merging', async () => {
    ...
    const hits = await federatedSearch('summary', null, { home, k: 2, decayTauMs: 1e15 });
    expect(hits).toHaveLength(2);
  });

  it('returns empty array when no workspaces are consented', async () => {
    await mkWs('not-yet', false);
    const hits = await federatedSearch('summary', null, { home });
    expect(hits).toEqual([]);
  });
});
