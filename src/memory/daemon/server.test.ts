  ...
  prepare(sql: string) {
      ...
      return Promise.resolve({
        run: (id: unknown, ts: unknown, cwd: unknown) => {
          if (!this.sessions.find((s) => s['id'] === id))
            this.sessions.push({ id, started_at: ts, cwd });
    ...
    if (stmt.includes('FROM summaries') && stmt.includes('WHERE id BETWEEN')) {
      return Promise.resolve({
        run: () => ({ lastInsertRowid: 0 }),
        get: () => Promise.resolve(undefined),
        all: (lo: unknown, hi: unknown) =>
          Promise.resolve(
            this.summaries.filter(
              (s) => (s['id'] as number) >= (lo as number) && (s['id'] as number) <= (hi as number),
            ),
          ),
      });
    }
    if (stmt.includes('FROM summaries WHERE id IN')) {
      return Promise.resolve({
        ...
        get: () => Promise.resolve(undefined),
        all: (...ids: unknown[]) =>
          Promise.resolve(this.summaries.filter((s) => ids.includes(s['id']))),
      });
    }
    ...
    if (stmt.includes("WHERE status = 'raw'")) {
      return Promise.resolve({
        ...
        all: (limit: unknown) =>
          Promise.resolve(
            this.events.filter((e) => e['status'] === 'raw').slice(0, limit as number),
          ),
      });
    }
    ...
    if (stmt.startsWith('INSERT OR REPLACE INTO summary_cache')) {
      return Promise.resolve({
        run: (k: unknown, t: unknown, ti: unknown, to: unknown) => {
          this.cache.set(k as string, {
            text: t as string,
            tokens_in: ti as number | null,
            tokens_out: to as number | null,
          });
          return { lastInsertRowid: 0 };
        },
        get: () => Promise.resolve(undefined),
...
describe('buildHandler', () => {
  ...
  it('drops captures for ignored paths (defaults backstop)', async () => {
    const h = buildHandler({ storage, wal, cwd: '/x' });
    const r = await h({
      kind: 'capture',
      sessionId: 's',
      tool: 'Read',
      payload: { tool_input: { file_path: '/x/node_modules/foo/index.js' } },
    });
    expect(r.ok).toBe(true);
    expect((r.data as { dropped?: boolean }).dropped).toBe(true);
    expect(db.events).toHaveLength(0);
  });

  it('captures non-ignored paths normally', async () => {
    const h = buildHandler({ storage, wal, cwd: '/x' });
    const r = await h({
      kind: 'capture',
      sessionId: 's',
      tool: 'Read',
      payload: { tool_input: { file_path: '/x/src/app.ts' } },
    });
    expect(r.ok).toBe(true);
    expect(typeof (r.data as { id?: number }).id).toBe('number');
    expect(db.events).toHaveLength(1);
  });
  ...
  it('returns search hits when summaries match', async () => {
    ...
    const h = buildHandler({ storage, wal, cwd: '/x' });
    const r = (await h({ kind: 'search', query: 'hello' })) as {
      ok: true;
      data: { hits: unknown[] };
    };
    expect(r.ok).toBe(true);
  ...
  it('uses default k when search is invoked without k', async () => {
    ...
    const h = buildHandler({ storage, wal, cwd: '/x' });
    const r = (await h({ kind: 'search', query: 'alpha' })) as {
      ok: true;
      data: { hits: unknown[] };
    };
    expect(r.ok).toBe(true);
  ...
  it('returns timeline rows around a near id', async () => {
    ...
    const h = buildHandler({ storage, wal, cwd: '/x' });
    const r = (await h({ kind: 'timeline', nearId: 1, window: 1 })) as {
      ok: true;
      data: { rows: unknown[] };
    };
    expect(r.ok).toBe(true);
  ...
  it('drain processes pending events and returns counts', async () => {
    ...
    const r = (await h({ kind: 'drain', batch: 4 })) as {
      ok: true;
      data: { processed: number; errors: number; backend: string };
    };
    expect(r.ok).toBe(true);
  ...
  it('drain marks events skipped when summarizer throws', async () => {
    ...
    const r = (await h({ kind: 'drain', batch: 4 })) as {
      ok: true;
      data: { errors: number; firstError?: string };
    };
    expect(r.ok).toBe(true);
  ...
  it('why returns empty edges when no provenance store', async () => {
    const h = buildHandler({ storage, wal, cwd: '/x' });
    const r = (await h({ kind: 'why', nodeKind: 'summary', nodeId: '1' })) as {
      ok: true;
      data: { edges: unknown[] };
    };
    expect(r.ok).toBe(true);
  ...
  it('why returns edges from the provenance store', async () => {
    const fakeProv = {
      trace: (node: { kind: string; id: string }, depth: number) => {
        return depth > 0
          ? [{ id: 1, from: node, to: { kind: 'b', id: '2' }, edgeType: 'causes' }]
          : [];
      },
    };
    const h = buildHandler({
    ...
    const r = (await h({ kind: 'why', nodeKind: 'summary', nodeId: '1', depth: 2 })) as {
      ok: true;
      data: { edges: unknown[] };
    };
    expect(r.ok).toBe(true);
...
describe('buildHandler with real Storage (cross-platform)', () => {
  ...
  it('serves the summaries RPC kind ordered most-recent-first', async () => {
    for (let i = 0; i < 4; i++) {
      const eid = await realStorage.recordEvent({
        ts: i,
        sessionId: 's',
        tool: 'R',
        payload: { i },
    ...
    const r = (await h({ kind: 'summaries', limit: 2 })) as {
      ok: true;
      data: { summaries: Array<{ text: string }> };
    };
    expect(r.ok).toBe(true);
  ...
  it('summaries truncates long text to 240 chars with an ellipsis', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'summaries' })) as {
      ok: true;
      data: { summaries: Array<{ text: string }> };
    };
  ...
  it('drain releases retryable failures back to raw with attempts incremented', async () => {
    ...
    const r = (await h({ kind: 'drain', batch: 1 })) as {
      ok: true;
      data: { processed: number; errors: number; pending: number };
    };
    expect(r.ok).toBe(true);
  ...
  it('drain pending count reflects the real backlog, not a 0/1 probe', async () => {
    ...
    const r = (await h({ kind: 'drain', batch: 1 })) as {
      ok: true;
      data: { processed: number; pending: number };
    };
    expect(r.data.processed).toBe(1);
  ...
  it('claim_for_summary returns claimed events and flips status to claimed', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'claim_for_summary', batch: 2 })) as {
      ok: true;
      data: { events: Array<{ id: number }> };
    };
    expect(r.ok).toBe(true);
  ...
  it('release_summary returns event to raw on retryable error', async () => {
    ...
    await realStorage.markEventStatus(eid, 'claimed');
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'release_summary', eventId: eid, error: 'timeout' })) as {
      ok: true;
      data: { status: string };
    };
  ...
  it('cache_get / cache_put round-trip', async () => {
    ...
    await h({
      kind: 'cache_put',
      cacheKey: 'k1',
      text: '{"text":"hello","confidence":0.8}',
      tokensIn: 5,
      tokensOut: 2,
    });
    const r = (await h({ kind: 'cache_get', cacheKey: 'k1' })) as {
      ok: true;
      data: { cached: { text: string; tokensIn: number | null } | null };
    };
  ...
  it('prune drops skipped events older than the cutoff', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'prune', maxAgeMs: 7 * 24 * 60 * 60 * 1000 })) as {
      ok: true;
      data: { removedEvents: number };
    };
  ...
  it('export RPC returns ndjson with at least one record per seeded table', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'export', all: true })) as {
      ok: true;
      data: { ndjson: string; records: number };
    };
    expect(r.ok).toBe(true);
  ...
  it('import RPC ingests ndjson with INSERT OR IGNORE semantics', async () => {
    ...
    const r2 = (await h({ kind: 'import', ndjson })) as {
      ok: true;
  ...
  it('savings RPC returns capture / drain / spend / context totals', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'savings' })) as {
      ok: true;
      data: {
        capture: { events: number };
        drain: { summarized: number };
        spend: { tokensIn: number; tokensOut: number };
        context: { netSavedTokens: number };
      };
    };
    expect(r.ok).toBe(true);
  ...
  it('doctor reranker probe is cached for 60s; heal=true bypasses cache', async () => {
    ...
    const a = (await h({ kind: 'doctor' })) as {
      ok: true;
      data: { reranker?: { ok: boolean; cached?: boolean } };
    };
    expect(pingCalls).toBe(1);
    expect(a.data.reranker?.cached).toBeUndefined();
    // Second call within 60s → cached, no new ping
    const b = (await h({ kind: 'doctor' })) as {
      ok: true;
      data: { reranker?: { cached?: boolean } };
    };
    expect(pingCalls).toBe(1);
  ...
  it('doctor heal=true triggers vec backfill when drift exists', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'doctor', heal: true })) as {
      ok: true;
      data: { vecCardinality: { drift: number }; healed?: { vecBackfilled: number } };
    };
    expect(r.ok).toBe(true);
  ...
  it('backfill RPC returns scanned/captured counts even with no transcripts', async () => {
    const h = buildHandler({
      storage: realStorage,
      wal: realWal,
      cwd: '/nonexistent-cwd-' + Date.now(),
    });
    const r = (await h({ kind: 'backfill', source: 'transcripts', workspaceOnly: true })) as {
      ok: true;
      data: { source: string; scanned: number; captured: number; errors: number };
    };
    expect(r.ok).toBe(true);
  ...
  it('compact RPC reports cache prune + vacuum results', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'compact', cacheMaxAgeMs: 30 * 24 * 60 * 60 * 1000 })) as {
      ok: true;
      data: { cachePruned: number; ftsRebuilt: boolean };
    };
  ...
  it('patterns RPC surfaces recurring buckets with isoformat timestamps', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'patterns', minRepeats: 3 })) as {
      ok: true;
      data: { patterns: Array<{ occurrences: number; firstTs: string }> };
    };
  ...
  it('session_digest RPC concats summaries with iso timestamps', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'session_digest', sessionId: 'sd' })) as {
      ok: true;
      data: { text: string; firstTs: string | null };
    };
  ...
  it('sessions RPC returns session metadata with isoformat timestamps', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'sessions' })) as {
      ok: true;
      data: { sessions: Array<{ sessionId: string; firstTs: string; eventCount: number }> };
    };
  ...
  it('dashboard RPC composes stats + doctor + pinned + patterns in one call', async () => {
    ...
    const sid = await realStorage.recordSummary({
      eventId: eid,
      ts: Date.now(),
      model: 'm',
      promptHash: 'p',
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'dashboard' })) as {
      ok: true;
      data: {
        stats: { counts: { events: number } };
        doctor: { integrity: string };
        pinned: unknown[];
        patterns: unknown[];
      };
    };
  ...
  it('as_of RPC returns counts + summaries filtered by ts cutoff', async () => {
    ...
    const eid2 = await realStorage.recordEvent({
      ts: t0 + 5000,
      sessionId: 's',
      tool: 'R',
      payload: { i: 2 },
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'as_of', ts: t0 + 1000 })) as {
      ok: true;
      data: { counts: { summaries: number }; summaries: Array<{ text: string }> };
    };
  ...
  it('auto_pin_patterns RPC reports pinned + patternsConsidered counts', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'auto_pin_patterns', minRepeats: 3 })) as {
      ok: true;
      data: { pinned: number; patternsConsidered: number };
    };
  ...
  it('context_budget RPC fills under the token cap and stops past it', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'context_budget', query: 'cat', maxTokens: 60 })) as {
      ok: true;
      data: { hits: Array<{ tokens: number }>; tokensUsed: number };
    };
    expect(r.ok).toBe(true);
  ...
  it('replay RPC returns events with summaries joined', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'replay', sessionId: 'rep' })) as {
      ok: true;
      data: { events: Array<{ summary: { text: string } | null }> };
    };
  ...
  it('symbol_search RPC returns events matching a symbol', async () => {
    const eid = await realStorage.recordEvent({
      ...
      sessionId: 's',
      tool: 'Edit',
      payload: { tool_input: { file_path: '/x.ts', content: 'x' } },
    });
    await realStorage.setEventSymbols(eid, ['function:login']);
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'symbol_search', query: 'function:login', k: 5 })) as {
      ok: true;
      data: { hits: Array<{ eventId: number }> };
    };
    expect(r.data.hits).toHaveLength(1);
  ...
  it('stats RPC returns throughput, backlog, top tools', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'stats', windowMs: 60_000 })) as {
      ok: true;
      data: {
        counts: { events: number };
        throughput: { eventsPerMin: number };
  ...
  it('thread RPC returns sessions sharing input_hash', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'thread', sessionId: 's1', limit: 10 })) as {
      ok: true;
      data: { sessions: Array<{ sessionId: string; sharedEvents: number }> };
    };
    expect(r.data.sessions).toHaveLength(2);
  ...
  it('thread RPC returns empty when the session has no shared inputs', async () => {
    ...
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'thread', sessionId: 'lonely' })) as {
      ok: true;
  ...
  it('backfill RPC rejects unsupported sources', async () => {
    const h = buildHandler({ storage: realStorage, wal: realWal, cwd: '/x' });
    const r = (await h({ kind: 'backfill', source: 'sftp' as 'transcripts' })) as {
      ok: false;
      error: string;
    };
    expect(r.ok).toBe(false);
  ...
  it('capture honors ttlMs by storing expires_at = ts + ttlMs', async () => {
    ...
    const row = (await (
      await (
        realStorage as unknown as {
          ['db']: { prepare(s: string): Promise<{ get(...p: unknown[]): Promise<unknown> }> };
        }
      )['db'].prepare('SELECT expires_at FROM events WHERE id = ?')
