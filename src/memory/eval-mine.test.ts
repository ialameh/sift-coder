...
async function seed(text: string): Promise<number> {
  ...
  storage.putEmbedding(sid, await e.embed(text));
  return sid;
}
...
describe('mineGolden', () => {
  it('returns empty array when no embeddings exist', () => {
    expect(mineGolden(storage)).toEqual([]);
  });
  ...
  it('produces (query, [id]) pairs using top-IDF terms from each summary', async () => {
    ...
    const items = mineGolden(storage, { termsPerQuery: 3 });
  ...
  it('skips summaries below minSummaryTokens', async () => {
    await seed('hi');
    expect(mineGolden(storage, { minSummaryTokens: 5 })).toEqual([]);
  });

  it('honors maxItems', async () => {
    for (let i = 0; i < 5; i++) await seed(`alpha beta gamma delta epsilon ${i}`);
    expect(mineGolden(storage, { maxItems: 2 })).toHaveLength(2);
  });
  ...
  it('skips summaries whose informative tokens are all empty', async () => {
    ...
    storage.putEmbedding(sid, await e.embed('the a of and or to'));
    const items = mineGolden(storage, { minSummaryTokens: 0 });
