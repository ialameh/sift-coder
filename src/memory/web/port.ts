...
export function chooseStablePort(workspaceKey: string): number {
  const h = createHash('sha256').update(workspaceKey).digest();
  const n = h.readUInt32BE(0);
  return PORT_RANGE_LOW + (n % PORT_RANGE_SIZE);
}

export function nextCandidate(prev: number): number {
...
export function initialPort(input: PickPortInput): PortChoice {
