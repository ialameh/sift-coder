 ...
 *   8. backfillFromV2 dry-run round-trip
 *
...
try {
  ...
  // 9. backfill dry-run round-trip
  // create a fake V1 db
  const fakeV1Root = path.join(SIFT_HOME, 'fake-v1-root');
  const fakeV1Db = path.join(fakeV1Root, 'workspaces', 'aaaaaaaaaaaa', 'memory.db');
  fs.mkdirSync(path.dirname(fakeV1Db), { recursive: true });
  // minimal schema + seed
  const Database = (await import('better-sqlite3')).default;
  const seedDb = new Database(fakeV1Db);
  const { CORE_DDL } = await import(path.join(ROOT, 'dist', 'memory', 'storage', 'schema.js'));
  seedDb.exec(CORE_DDL);
  seedDb.prepare("INSERT INTO sessions (id, started_at, cwd) VALUES (?, ?, ?)").run('s1', Date.now(), '/x');
  seedDb.prepare(
    "INSERT INTO events (ts, session_id, tool, input_hash, payload_json, status, tokens_est) VALUES (?, 's1', 'Read', 'h1', '{}', 'raw', 10)"
  ).run(Date.now());
  seedDb.close();

  const { backfillFromV2 } = await import(path.join(ROOT, 'dist', 'memory', 'migration', 'v2-import.js'));
  const dry = await backfillFromV2({ v1Root: fakeV1Root, v3Ns: NS, dryRun: true });
  record('backfill_dry_run', dry.errors.length === 0 && dry.totals.events === 1, dry.totals);

  const real = await backfillFromV2({ v1Root: fakeV1Root, v3Ns: NS, dryRun: false });
  record('backfill_real_run', real.errors.length === 0 && real.totals.events === 1, real.totals);

  // verify v3 db has the event
  const v3Db = path.join(SIFT_HOME, '.siftcoder', NS, 'workspaces', 'aaaaaaaaaaaa', 'memory.db');
  const probe = new Database(v3Db);
  const cnt = probe.prepare('SELECT COUNT(*) as c FROM events').get().c;
  probe.close();
  record('backfill_target_db_has_event', cnt === 1, { count: cnt });
