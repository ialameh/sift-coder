 ...
 */
async function validateCheckpoint(data, name) {
    const errors = [];
 ...
 */
async function captureFileIterationState() {
 ...
 */
export async function saveCheckpoint(name, data) {
    try {
        // Validate input
 ...
 */
async function restoreFileIterationState(fileIteration) {
    if (!fileIteration)
        return;
    const iterationStatePath = path.join(STATE_DIR, 'file-iteration.json');
    try {
 ...
 */
async function updateIndex(checkpoint) {
