 ...
 */
export async function initializeIteration(pattern, goal, checkpointEvery = 10) {
    try {
 ...
 */
export async function advanceToNextFile(state) {
...
export async function addInsight(insight) {
    try {
        ...
        data.files_analyzed = data.insights.reduce((count, ins) => {
            return count + (data.insights.filter((i) => i.source === ins.source).length > 0 ? 1 : 0);
        }, 0);
 ...
 */
export async function shouldCheckpoint(state) {
 ...
 */
export async function createIterationCheckpoint(state, reason = 'periodic') {
 ...
 */
export async function pauseIteration() {
    const state = await loadState();
    if (!state) {
...
//# sourceMappingURL=file-iterator.js.map