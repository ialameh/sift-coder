...
export class MockFileSystem {
    ...
    createFsPromisesMock() {
        ...
        // Mock mkdir
        fs.mkdir.mockImplementation((_path, _options) => {
            return Promise.resolve();
        });
        // Mock readdir
        ...
        // Mock appendFile
        fs.appendFile.mockImplementation((path, content) => {
...
export class TestDataFactory {
     ...
     */
    static createBoundaries(overrides) {
        return {
            modifiable: ['src/**/*'],
     ...
     */
    static createCurrentTask(overrides) {
        return {
            feature: 'feat-123456',
            subtask: 'subtask-1',
     ...
     */
    static createPattern(overrides) {
        return {
            id: 'pattern-1',
            title: 'Test Pattern',
            source: 'test',
...
export class MockProcessExecutor {
    ...
    createSpawnMock() {
        ...
        spawn.mockImplementation((command, args, _options) => {
            const mockChild = {
                stdout: {
...
export class AssertionHelpers {
     ...
     */
    static assertFileWritten(fs, path, content) {
     ...
     */
    static assertFileDeleted(fs, path) {
        expect(fs.hasFile(path)).toBe(false);
    }
    /**
     * Assert that directory was created
     */
    static assertDirectoryCreated(fs, path) {
        expect(fs.hasDirectory(path)).toBe(true);
    }
    /**
     * Assert that command was executed
     */
    static assertCommandExecuted(execMock, command) {
        expect(execMock).toHaveBeenCalledWith(expect.stringContaining(command), expect.anything(), expect.anything());
    }
}
/**
