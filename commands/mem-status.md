...
If `events captured > 0` but `summarized: 0`, do NOT blindly suggest `mem_drain`. As of 2026-04-30, Claude Code 2.1.x does not implement MCP `sampling/createMessage`, so `mem_drain` returns "Method not found" against the default config. Tell the user: **"Capture working, drain needs a backend. Three paths: (a) install Ollama (`brew install ollama && ollama pull llama3.2:3b && ollama serve`) — free, private, auto-detected. (b) Set `SIFTCODER_DRAIN_BACKEND=anthropic` + `ANTHROPIC_API_KEY` for paid Anthropic API. (c) Stay raw-only — `mem_search` already works against captured payloads. See `/siftcoder:memory:drain` docs."**
