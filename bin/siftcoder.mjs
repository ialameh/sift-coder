...
switch (cmd) {
  ...
  case 'backfill': {
    if (args[0] === '--from-v2' || args[0] === '--from-v1') {
      const v1Root = args[1] || path.join(os.homedir(), '.siftcoder');
      const dryRun = args.includes('--dry-run');
      const wsKey = (() => {
        const i = args.indexOf('--workspace');
        return i >= 0 ? args[i + 1] : undefined;
      })();
      const mod = await import(path.join(ROOT, 'dist', 'memory', 'migration', 'v2-import.js'));
      const result = await mod.backfillFromV2({ v1Root, dryRun, workspaceKey: wsKey });
